...
int main(){
  ...
  // 1) Uniform policy
  cfg.level_policy = P4estBuilder::Policy::uniform(6);
  std::cout << "[ex02] Uniform policy L=6\n";
  P4estBuilder::build_forest(H, cfg);

  // 2) Linear by leaf count between L=4..10
  cfg.level_policy = P4estBuilder::Policy::by_leafcount_linear(H, cfg.n, 4, 10);
  std::cout << "[ex02] by_leafcount_linear L in [4,10]\n";
  P4estBuilder::build_forest(H, cfg);

  // 3) Quantile bands by count: 20% (low=4), middle (7), 80% (high=10)
  cfg.level_policy = P4estBuilder::Policy::by_leafcount_quantiles(H, cfg.n, 0.2, 0.8, 4, 7, 10);
  std::cout << "[ex02] by_leafcount_quantiles q_lo=0.2 q_hi=0.8\n";
  P4estBuilder::build_forest(H, cfg);

  // 4) Bands by mean probability
  cfg.level_policy = P4estBuilder::Policy::bands_by_mean_prob(H, cfg.n, {0.3, 0.6, 0.85}, {4, 7, 9, 11});
  std::cout << "[ex02] bands_by_mean_prob thresholds 0.3/0.6/0.85\n";
  P4estBuilder::build_forest(H, cfg);

  // 5) Opaque forest handle with a given policy
  cfg.level_policy = P4estBuilder::Policy::by_mean_prob_threshold(H, cfg.n, 0.5, 6, 9);
  auto* fh = P4estBuilder::build_forest_handle(H, cfg);
  std::cout << "[ex02] forest handle created (and will be destroyed)\n";
  delete fh;
#else
  std::cout << "[ex02] OCTOWEAVE_WITH_P4EST is OFF; skipping real forest build.\n";
#endif
  return 0;
}
